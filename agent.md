...
### 13. `.adonisjs/` is codegen, and a stale compiled copy shadows it

`.adonisjs/server/controllers.ts`, `pages.d.ts` and the Tuyau client registry are regenerated by the assembler's `init` hooks — that is, whenever `node ace serve` or `node ace build` starts. There is no ace command for it. After adding a controller or route, start the dev server once (twice if a route names a controller that was not in the index yet) so the registry and `controllers.ts` catch up; until then the typed client does not know the new endpoint.
...
```bash
ADMIN_DB=postgresql://postgres:postgres@localhost:5432/boss_man_test_b PORT=4452 npm test
```
