...
export default class Lease extends compose(SuperBaseModel, Auditable) {
  ...
  @hasMany(() => Reference) declare references: HasMany<typeof Reference>
  ...
  @afterCreate()
  public static async createCalEvent(lease: Lease) {
    if (lease.endDate) {
      const date = DateService.formatDate(lease.endDate, true)
      await CalendarEvent.create(
        {
          allDay: true,
          title: `Lease Expiry for ${lease.name}`,
          category: 'lease-expiry',
          description: `
          The ${lease.name} expires on ${date}.
         You can renew the lease, terminate it or let it expire.
        -Auto generated by the system`,
          startDate: lease.endDate,
          endDate: lease.endDate,
          orgId: lease.orgId,
          metadata: { lease: { id: lease.id, autogenerated: true } },
        },
        { client: lease.$trx },
      )

      logger.info(`Lease expiry event for ${lease.name} has been created`)
    }
  }
